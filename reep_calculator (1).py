...
with tab_large:
    ...
    with inp_col:
        ...
        custom_gpm = None
        if use_cust:
            custom_gpm = st.slider("Custom GPM", min_value=0.10, max_value=0.75, value=0.32, step=0.01, format="%.0f%%", key="lg_gpm")

        st.markdown('<div class="hr"></div>', unsafe_allow_html=True)
        st.markdown('<div class="lbl">Deck Over GPM</div>', unsafe_allow_html=True)
        deck_gpm = st.slider("Deck GPM", min_value=0.10, max_value=0.75, value=0.33, step=0.01, format="%.0f%%", key="lg_deck_gpm")
...
with tab_small:
    ...
    with sl:
        ...
        s_custom_gpm = None
        if s_use_cust:
            s_custom_gpm = st.slider("Custom GPM", min_value=0.10, max_value=0.75, value=0.32, step=0.01, format="%.0f%%", key="sm_gpm")

        st.markdown('<div class="hr"></div>', unsafe_allow_html=True)
        st.markdown('<div class="lbl">Deck Over GPM</div>', unsafe_allow_html=True)
        s_deck_gpm = st.slider("Deck GPM", min_value=0.10, max_value=0.75, value=0.33, step=0.01, format="%.0f%%", key="sm_deck_gpm")
...
with tab_repair:
    ...
    with rl:
        ...
        r_custom_gpm = None
        if r_use_cust:
            r_custom_gpm = st.slider("Repair Custom GPM", min_value=0.10, max_value=0.75, value=0.60, step=0.01, format="%.0f%%", key="rep_gpm")
